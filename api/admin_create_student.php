...
// Allow admin and professors to create students via this endpoint
...
try {
    ...
    if ($user) {
        ...
        $ins->execute([':username' => $username, ':password' => $password, ':email' => $email, ':first' => $first, ':last' => $last]);
        $student_db_id = $db->lastInsertId();
    }

    // Optionally enroll in a course if course_id provided
    if ($course_id) {
    ...
    echo json_encode(['success' => true, 'student_db_id' => (int)$student_db_id, 'message' => 'Student created or found.']);
    exit;
} catch (Exception $e) {
    error_log('admin_create_student error: ' . $e->getMessage());
